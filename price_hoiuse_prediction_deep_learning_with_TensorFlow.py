# simplistic example of predicting hause prices based on hause size
...
np.random.seed(42)
# *100 so that there is a relationship between size and price
house_price = house_size * 100 + np.random.randint(low=20000, high=70000, size=num_house)

# plot a graph of size to price
# plt.plot(house_size, house_price, "bx")
# plt.ylabel("price")
# plt.xlabel("size")
# plt.show()

# for standartize data, use standart deviation to prevent over/underflow
def normalize(array):
...
# Define the variables holding the size_factor, weight and bias offset
tf_size_factor = tf.Variable(np.random.randn(), name="size_factor")
tf_price_offset = tf.Variable(np.random.randn(), name="price_offset")


# ceate the relation Function
...
with tf.Session() as sess:
   ...
   # Plot of training and test data, and learned regression
    ...
    def animate(i):
     ...
     # make an animation to show how the model learns
    def initAnim():
